-- Set existing seed templates to full-funnel
UPDATE public.seed_templates SET template_type = 'full-funnel' WHERE template_type IS NULL;

-- Update handle_new_user_role to copy template_type
...
BEGIN
  ...
  FOR tmpl IN
    SELECT st.name, st.description, st.nodes, st.edges, st.brief_structure, st.template_type
    FROM public.seed_templates st
    WHERE st.is_active = true
  LOOP
    INSERT INTO public.funnels (user_id, sub_account_id, name, description, nodes, edges, is_template, template_type)
    VALUES (NEW.id, new_sub_id, tmpl.name, tmpl.description, tmpl.nodes, tmpl.edges, true, tmpl.template_type)
