...
ALTER TABLE public.funnels ADD COLUMN IF NOT EXISTS seed_template_id uuid;

CREATE POLICY "Authenticated can read active seed templates"
...
BEGIN
  ...
  FOR tmpl IN
    SELECT st.id, st.name, st.description, st.nodes, st.edges, st.brief_structure, st.template_type
    FROM public.seed_templates st
    WHERE st.is_active = true
  LOOP
    INSERT INTO public.funnels (user_id, sub_account_id, name, description, nodes, edges, is_template, template_type, seed_template_id)
    VALUES (NEW.id, new_sub_id, tmpl.name, tmpl.description, tmpl.nodes, tmpl.edges, true, tmpl.template_type, tmpl.id)
